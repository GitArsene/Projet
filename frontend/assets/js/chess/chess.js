...
// Se connecter au serveur WebSocket sur ws://localhost:8082
...
if (!roomId) {
  ...
  window.location.href = "../../frontend/chess/chessRoom.html";
}
...
ws.onmessage = (event) => {
  ...
  if (data.type === "joined") {
    ...
    window.location.href = "/frontend/chess/chessRoom.html"; // Retour au lobby en cas d'erreur
...
function updateGridHTML(board) {
  ...
  document.getElementById(
    "currentPlayer"
  ).textContent = `${currentPlayerColor}`; // Update the current player text
}
    ...
    for (let y = 0; y < board.length; y++) {
      ...
      const moveDrag = (event) => {
        if (isDragging) {
          let clickX =
            ...
            window.scrollX; // Adjust X coordinate with scroll offset
          let clickY =
            (event.changedTouches?.[0]?.clientY || event.clientY) +
            window.scrollY; // Adjust Y coordinate with scroll offset
      ...
      const stopDrag = async (event) => {
        if (isDragging) {
          ...
          const hoveredElementId = document.elementFromPoint(clickX, clickY)
            ?.parentElement?.id;
          if (hoveredElementId) {
            // Handle the square click and update the last clicked square
            if (
              hoveredElementId !== `${x},${y}` &&
              /^[0-9]+,[0-9]+$/.test(hoveredElementId)
            ) {
              let hoveredClick = {
                x: Number(hoveredElementId[0]),
                y: Number(hoveredElementId[2]),
              }; // Get the coordinates of the square under the mouse pointer
              isDragging = false;
              times = 0;
      ...
      if (
        board[x][y] !== undefined &&
        board[x][y].color === currentPlayerColor &&
